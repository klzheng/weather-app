...
export default function Background({ children, data }) {
  ...
  const getBackgroundImg = useCallback(
    (weatherCode) => {
      setLoading(true);
  ...
  useEffect(() => {
    if (Object.keys(data).length !== 0) getBackgroundImg("test");
  ...
  return (
    <>
      ...
      <div
        className={
          "fixed inset-0 bg-fill w-100vw h-100vh text-white overflow-y-auto overflow-x-hidden bg-gradient-to-b from-gray-600 to-gray-900 transition-all "
        }
      >
        {/* Container (appears when loading) */}
        <div
          className={
            "flex flex-col items-center 2xs:w-76 xs:w-112 sm:w-128  bg-black bg-opacity-30 rounded-3xl backdrop-blur-md shadow-dark border border-slate-900 border-opacity-5 py-5 my-5 transition-all 2xs:mx-auto  2xs:h-216 sm:h-180 " +
            (loading ? " opacity-100 animate-pulse " : " hidden ")
          }
        ></div>
        ...
        <div
          className={
            " transition-all  " +
            (loading ? " opacity-0 hidden -z-20 " : " opacity-100 ")
          }
        >
          <video
            src={bg}
