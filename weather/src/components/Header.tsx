...
export const Header = ({ data, address }: HeaderProps) => {
  return (
    <>
      {Object.keys(data).length !== 0 && (
        <section className=" w-104">
          <div className="sm:flex sm:flex-row 2xs:flex-col gap-x-8 2xs:space-y-5 sm:gap-y-0 justify-center items-center">
            <div className="col-span-1 flex items-center justify-center">
              <span className="text-6xl tracking-tight">
                {data[0].values.temperature.toFixed(0)}°
              </span>
            </div>
          ...
          <div className=" flex flex-row items-center justify-center text-sm font-light mt-12 space-x-4 text-gray-300">
            <span>
              <i className="fa-solid fa-umbrella"></i> Precipitation:{" "}
              {data[0].values.precipitationProbability}%{" "}
              {data[0].values.precipitationIntensity !== 0 &&
                `(${data[0].values.precipitationIntensity} in)`}
            </span>
            <span>
              <i className="fa-solid fa-wind"></i> Wind:{" "}
              {data[0].values.windSpeed} mph
            </span>
          </div>
